...
int main(int argc, char *argv[])
{
    ...
    if(adjusted_length + hole + 9 < mmap_size_in_pages * PageSize)
    ...
    /* Modificăm parțial conținutul mapării, mai precis începând de la hole=5 octeți după finalul porțiunii mapate din fișier. */
    for (i = 0; i < 9; i++) {  *(map_addr + adjusted_length + hole + i) = 'X'+(i/3) ; }
    /* Notă: mai concis am fi putut scrie astfel:
       strncpy(map_addr+adjusted_length+hole,"XXXYYYZZZ_!",9);  // Copie doar primele 9 caractere (deci fără caracterul null), la acea adresă din cadrul mapării.
       ..., dar apelul strncpy generează un warning la compilarea cu opțiunea -Wall și, în plus,
       am preferat să vă arăt cum modificăm efectiv adrese de mmemorie individuale din cadrul mapării. */

    /* Practic, pe cele 5 poziții, din cadrul mapării, de la adresele: map_addr+adjusted_length+j, cu j=0,1,2,3,4, vor rămâne zero-urile scrise la crearea mapării!
       Aceste 5 zerouri nu vor fi vizibile la afișarea pe ecran, de mai jos!
    ...
    /* Scriem pe ecran (stdout) conținutul mapării. Mai precis, afișăm doar zona mapată din fișier, plus porțiunea modificată în memorie,
       de după poziția corespunzătoare EOF-ului din fișier ! */
    printf("\nThe 2nd modified content of the mapping, started from the aligned offset, is the following:\n");
    codw = write(STDOUT_FILENO, map_addr, adjusted_length + hole + 9);
    if (codw == -1)
        handle_error("Error at 3rd write on screen",8);
    if (codw != adjusted_length + hole + 9) {
