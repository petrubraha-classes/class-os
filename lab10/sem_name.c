...
int main(int argc, char *argv[])
{
  ...
  if (SEM_FAILED == my_semaphore)
    handle_error("Error at sem_open", 3);

  for (int i = 1; i <= nr_iterations; i++)
  {
    /* Entry section:  wait(semaphore); */
    if (-1 == sem_wait(my_semaphore))
      handle_error("Error at sem_wait", 4);

    /* The critical section. */
    critical_section(i);

    /* Exit section:  signal(semaphore); */
    if (-1 == sem_post(my_semaphore))
      handle_error("Error at sem_post", 5);
  ...
  if (-1 == sem_close(my_semaphore))
    handle_error("Error at sem_close", 6);
